...
def SingleEvaluation(features_file, labels_file):
    ...
    # In the following part we design a buy_or_sell trading stategy based on the prediction.
    ...
    for i in range(length_y - 1):
        if predictions[i+1] < y[i]:
            diffence = y[i] - y[i+1]
        elif predictions[i+1] >= y[i]:
            diffence = y[i+1] - y[i]

        strategy_earning += (diffence + tran_cost_rate * (y[i+1] + y[i])) / y[i]

    strategy_earning = 100 * strategy_earning

    print('Strategy earning in this period is:', strategy_earning)    
